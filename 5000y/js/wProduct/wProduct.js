...
//		alert("点击:"+$(this).text());
			   ...
			   $(".Case_CanPing").show();
			     $(".Case_DanCanPing").hide();
					 $.ajax({
							...
							url:"http://picp.zicp.net/wuqianyan/wProduct_WgetTypeproduct",
							dataType: 'jsonp',			
							jsonp: "callback",
							data:{productType:$(this).text()},
	...
	/*加载单个产品*/
			$(".Case_CanPing").delegate('li','click',function(){//产品遮罩
			  ...
			  $.ajax({
                ...
                url:"http://picp.zicp.net/wuqianyan/wProduct_WgetProduct",
                dataType: 'jsonp',			
				jsonp: "callback",
				data:{productName:$(this).find('.mark a').attr("alt")},
                beforeSend:function(XMLHttpRequest){
				...
				$(".Case_CanPing").hide(1000);
                 	...
                 	$(".Case_DanCanPing").show(1000).html(data.json[0].productText+'<div class="Case_CanPing_show backButton"><a href="javascript:void(0);">返回</a></div>');
	...
	/*加载上边菜单项*/
	  $.ajax({
                ...
                url:"http://picp.zicp.net/wuqianyan/wProduct_WgetProducttype",
                dataType: 'jsonp',			
				jsonp: "callback",
                beforeSend:function(XMLHttpRequest){
                ...
                success:function (data){
					...
					for(var i=0;i<data.json.length;i++){
						  caseContent += '<li><a href="javascript:void(0);">'+data.json[i].productType+'</a></li>';
					}
			 ...
			 $.ajax({
                ...
                url:"http://picp.zicp.net/wuqianyan/wProduct_WgetAllproduct",
                dataType: 'jsonp',			
				jsonp: "callback",
                beforeSend:function(XMLHttpRequest){
                    $(".Case_CanPing").html("<center><img src=\"img/loading.gif\" width=\"32px\" height=\"32px\"/></center>");
	...
	/*初始化，显示第一页*/
	 ...
	 if(pageSize<list_length){//每页数小于总项数
		for(var i=pageCurrent*pageSize-pageSize;i<pageCurrent*pageSize;i++){
			    caseContent += '<li class="block" ><div><img  src="'+data[i].productImg+'" class="lazy" style="display: block;"></div><div class="mark" style="top: -207px;"><a href="javascript:void(0);" alt="'+data[i].productName+'">'+data[i].productName+'</a></div></li>';
				}
	}else{
